  ...
  <div class="page">
    <div class="page-header">
      <div>
        <h2>财务报表</h2>
        <p class="sub">
          按顶部场馆 + 日期筛选。营业额 = 区间内发卡实收 + 订场微信支付。
          实际收入 = 区间内到场消费：订场按场地价；次卡按「卡价÷次数」；时间卡按「卡价÷有效天数 × 实际用卡天数」
        </p>
      </div>
...
function fmtNum(n) { return (Math.round((Number(n) || 0) * 100) / 100).toFixed(2) }
const fmt = fmtNum
...
function setRange(type) {
  const now = new Date()
  if (type === 'week') { startDate.value = ymd(startOfWeek(now)); endDate.value = ymd(now) }
  else if (type === 'month') { startDate.value = ymd(new Date(now.getFullYear(), now.getMonth(), 1)); endDate.value = ymd(now) }
  else { const s = new Date(now); s.setDate(s.getDate() - 6); startDate.value = ymd(s); endDate.value = ymd(now) }
  loadData()
}
...
async function loadData() {
  if (!startDate.value || !endDate.value) { ElMessage.warning('请选择日期'); return }
  loading.value = true
  try {
...
function exportCsv() {
  ...
  daily.value.forEach((r) => { lines.push([r.date, fmt(r.turnover), fmt(r.income)].join(',')) })
...
onMounted(() => { setRange('month'); window.addEventListener('venue-changed', loadData) })
...
.stat-card { background: #fff; border-radius: 10px; padding: 16px 18px; margin-bottom: 12px; border-top: 3px solid #1a5c3a; box-shadow: 0 1px 4px rgba(0,0,0,.04); }
...
.card { background: #fff; border-radius: 10px; padding: 14px 16px 18px; margin-bottom: 14px; }
