  ...
  <div class="page">
    ...
    <el-table :data="list" stripe border v-loading="loading" size="small" class="detail-table">
      ...
      <el-table-column label="区间累计" width="90" align="center">
        <template #default="{ row }"><span class="em">{{ row.coachTotalHours || 0 }}h</span></template>
      </el-table-column>
      <el-table-column prop="time" label="时间段" width="110" />
      <el-table-column label="本节" width="70" align="center">
        <template #default="{ row }"><span :class="row.hours > 0 ? 'em' : 'muted'">{{ row.hours || 0 }}h</span></template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="88" align="center">
        <template #default="{ row }">
          <el-tag size="small" :type="row.attendStatus === 'completed' ? 'success' : row.attendStatus === 'cancelled' ? 'info' : 'warning'">{{ row.statusText }}</el-tag>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
function setRange(type) {
  ...
  if (type === 'week') {
    ...
    filters.startDate = fmt(new Date(now.getFullYear(), now.getMonth(), 1))
    filters.endDate = fmt(now)
  } else {
...
function exportExcel() {
  ...
  downloadCsv(`出勤明细_${filters.startDate}_${filters.endDate}.csv`,
    ['日期', '教练', '区间累计课时', '时间段', '本节课时', '课程', '门店', '场地', '学员', '手机号', '状态'],
    list.value.map((r) => [r.date, r.coachName, r.coachTotalHours || 0, r.time, r.hours || 0, r.courseName, r.venueName, r.court, r.userName, r.phone, r.statusText])
  )
  ElMessage.success('明细已导出')
}
function exportCoachSummary() {
  if (!coachStats.value.length) return ElMessage.warning('没有汇总')
  downloadCsv(`工资汇总_${filters.startDate}_${filters.endDate}.csv`,
    ['教练', '区间累计出勤课时', '已完成节数', '待上课', '已取消'],
    coachStats.value.map((r) => [r.coachName, r.completedHours, r.completedCount, r.pendingCount, r.cancelledCount])
  )
  ElMessage.success('工资汇总已导出')
}
function onVenueChanged() { loadCoaches(); loadData() }
onMounted(() => { loadCoaches(); loadData(); window.addEventListener('venue-changed', onVenueChanged) })
...
.period { margin: 6px 0 0; font-size: 13px; color: #606266; }
.period-tip { color: #909399; font-size: 12px; }
.actions { display: flex; gap: 8px; flex-wrap: wrap; }
.filters { background: #fff; padding: 10px 14px 0; border-radius: 8px; margin-bottom: 14px; }
.coach-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; margin-bottom: 16px; }
.coach-card { background: #fff; border: 1px solid #e4e7ed; border-radius: 10px; padding: 14px 16px; border-left: 4px solid #1a5c3a; }
.cname { font-size: 14px; color: #303133; font-weight: 600; }
.chours { font-size: 28px; font-weight: 700; color: #1a5c3a; line-height: 1.2; margin: 6px 0 4px; }
.chours .unit { font-size: 14px; font-weight: 600; margin-left: 2px; }
.cmeta { font-size: 12px; color: #909399; }
.detail-head { display: flex; align-items: baseline; gap: 12px; margin-bottom: 8px; font-size: 14px; font-weight: 600; color: #303133; }
.detail-tip { font-size: 12px; font-weight: 400; color: #909399; }
.detail-table { background: #fff; }
.em { color: #1a5c3a; font-weight: 700; }
.muted { color: #c0c4cc; }
</style>
