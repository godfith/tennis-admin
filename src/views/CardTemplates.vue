<template>
  <div class="page">
    ...
    <el-table :data="list" stripe border v-loading="loading">
      ...
      <el-table-column label="时间规则" min-width="160">
        <template #default="{ row }">
    ...
    <el-dialog v-model="visible" :title="form._id ? '编辑卡模板' : '新增卡模板'" width="520px" destroy-on-close>
      <el-form label-width="100px">
        ...
        <template v-if="form.type === 'times' || form.type === 'coach'">
          ...
          <el-form-item label="有效天数">
            <el-input-number v-model="form.durationDays" :min="0" placeholder="0表示不限" />
        ...
        <template v-if="form.type === 'time'">
          ...
          <el-form-item label="可用模式">
            <el-radio-group v-model="form.timeRule.mode">
              <el-radio value="all">全天不限</el-radio>
              <el-radio value="weekday">仅工作日</el-radio>
              <el-radio value="custom">自定义时段</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item v-if="form.timeRule.mode !== 'all'" label="开始时间">
            <el-time-select
              v-model="form.timeRule.startTime"
              start="06:00"
              step="00:30"
              end="22:00"
              placeholder="开始"
          ...
          <el-form-item v-if="form.timeRule.mode !== 'all'" label="结束时间">
            <el-time-select
              v-model="form.timeRule.endTime"
              start="06:00"
              step="00:30"
              end="23:00"
              placeholder="结束"
...
function timeRuleText(rule) {
  if (!rule) return '-'
  if (rule.mode === 'all') return '全天不限'
  if (rule.mode === 'weekday') return `工作日 ${rule.startTime || ''}-${rule.endTime || ''}`
...
function openEdit(row) {
  form.value = {
    ...
    timeRule: {
      mode: row.timeRule?.mode || 'all',
      startTime: row.timeRule?.startTime || '09:00',
      endTime: row.timeRule?.endTime || '18:00'
    },
