<template>
  <div class="page">
    <div class="page-header">
      <div>
        <h2>员工管理</h2>
        <p class="tip">当前场馆：{{ venueName || '未选择' }} · 仅前台 / 客服 / 店长等行政岗 · 教练请在「教练管理」维护</p>
      </div>
      <div>
      ...
      <el-form label-width="90px">
        ...
        <el-form-item label="岗位" required>
          <el-select v-model="form.role" style="width: 100%">
            <el-option label="前台" value="front" /><el-option label="客服" value="service" /><el-option label="店长" value="manager" /><el-option label="其他" value="other" />
          </el-select>
        </el-form-item>
...
const form = ref({ _id: '', name: '', phone: '', role: 'front', specialty: '', sort: 0, active: true, remark: '' })
...
function roleLabel(r) { return { front: '前台', service: '客服', manager: '店长', other: '其他' }[r] || r }
function roleTag(r) { return { front: 'success', service: 'primary', manager: 'danger', other: 'info' }[r] || 'info' }
...
function openAdd() {
  ...
  form.value = { _id: '', name: '', phone: '', role: 'front', specialty: '', sort: 0, active: true, remark: '' }
  visible.value = true
}
function openEdit(row) {
  form.value = { _id: row._id, name: row.name || '', phone: row.phone || '', role: row.role === 'coach' ? 'other' : row.role || 'other', specialty: row.specialty || '', sort: row.sort || 0, active: row.status === 'active', remark: row.remark || '' }
  visible.value = true
}
async function save() {
