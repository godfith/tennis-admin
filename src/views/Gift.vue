<template>
  <div class="page">
    <div class="page-header">
      <div>
        <h2>赠课 / 赠次</h2>
        <p class="sub">仅管理员可用。赠送记 0 元流水，不计入营业额，用户可直接用来订场或报团课。</p>
      </div>
    </div>

    <el-alert
      v-if="!canGift"
      title="当前账号无权赠送，请用管理员账号登录。"
      type="warning"
      show-icon
      :closable="false"
    ...
    <el-row :gutter="16">
      <el-col :xs="24" :md="10">
        <div class="card">
          <div class="card-title">赠送</div>
          <el-radio-group v-model="mode" style="margin-bottom: 14px">
            <el-radio-button label="new">发一张赠送卡</el-radio-button>
            <el-radio-button label="add">给已有卡加次</el-radio-button>
          </el-radio-group>
          ...
          <el-form label-width="96px">
            ...
            <el-form-item v-if="hits.length" label="选择">
              <el-checkbox-group v-model="pickedIds" class="hit-list">
                <el-checkbox v-for="u in hits" :key="u._id" :label="u._id">
                  {{ displayName(u) }} · {{ u.phone || '无手机' }}
                </el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            ...
            <template v-if="mode === 'new'">
              ...
              <el-form-item label="次数" required>
                <el-input-number v-model="form.times" :min="1" :max="99" />
              </el-form-item>
              ...
              <el-form-item label="卡名称">
                <el-input v-model="form.cardName" :placeholder="defaultName" />
              </el-form-item>
              <el-form-item label="生效">
                <el-date-picker v-model="form.validFrom" type="date" value-format="YYYY-MM-DD" />
              </el-form-item>
              <el-form-item label="到期">
                <el-date-picker v-model="form.validTo" type="date" value-format="YYYY-MM-DD" placeholder="可留空" />
              </el-form-item>
            </template>

            <template v-else>
              <el-form-item label="已有卡" required>
                <el-select v-model="form.cardId" placeholder="先勾一名用户再加载持卡" style="width: 100%">
                  <el-option
                    v-for="c in userCards"
                    :key="c._id"
                    :label="`${c.cardName}（剩${c.remainingTimes}/${c.totalTimes}）`"
                    :value="c._id"
                  />
                </el-select>
              </el-form-item>
            ...
            <el-form-item label="备注">
              <el-input v-model="form.remark" type="textarea" :rows="2" placeholder="如：新客体验 / 活动赠送" />
            </el-form-item>
            <el-form-item>
              <el-button type="primary" :loading="saving" :disabled="!canGift" @click="submit">确认赠送</el-button>
            </el-form-item>
          </el-form>
        </div>
      </el-col>
...
const defaultName = computed(() => {
  const map = { times: '赠送场地体验', group: '赠送团课体验', coach: '赠送私教体验' }
  return (map[form.value.type] || '赠送体验') + form.value.times + '次'
})

function displayName(u) {
  return (u && (u.nickName || u.userId || u._id)) || '-'
}
function typeLabel(t) {
  return { times: '场地', group: '团课', coach: '私教', time: '时间卡' }[t] || t
}
...
function pickedUsers() {
  return hits.value.filter((u) => pickedIds.value.includes(u._id))
}
...
watch(pickedIds, async (ids) => {
  form.value.cardId = ''
  userCards.value = []
  if (mode.value !== 'add' || ids.length !== 1) return
  const result = await post('/adminGift', authBody({ action: 'userCards', userId: ids[0] }))
  userCards.value = result.list || []
})
...
async function loadHistory() {
  ...
  try {
    const result = await post('/adminGift', authBody({ action: 'list' }))
...
async function submit() {
  if (!canGift) {
    ElMessage.warning('仅管理员可赠送')
    return
  }
  const users = pickedUsers()
  if (!users.length) {
    ElMessage.warning('请先搜索并勾选用户')
    return
  }
  saving.value = true
  try {
    ...
    if (mode.value === 'add') {
      if (users.length !== 1) {
        ElMessage.warning('给已有卡加次一次只能选一个人')
        return
      }
      if (!form.value.cardId) {
        ElMessage.warning('请选择要加次的卡')
        return
      }
      result = await post('/adminGift', authBody({
        action: 'addTimes',
      ...
      result = await post('/adminGift', authBody({
        ...
        cardName: form.value.cardName || defaultName.value,
    ...
    if (!result.ok) {
      ElMessage.error(result.msg || '赠送失败')
      return
    }
    ElMessage.success(mode.value === 'add' ? '已加次' : '已赠送 ' + (result.count || users.length) + ' 人')
    pickedIds.value = []
    form.value.remark = ''
    loadHistory()
  } catch (e) {
...
.hit-list { display: flex; flex-direction: column; gap: 6px; max-height: 180px; overflow: auto; }
</style>
