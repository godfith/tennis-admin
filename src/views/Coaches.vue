...
function isActive(row) {
  return row.status === 'active' || row.status === '在职' || row.status === 1 || row.status === true
}
...
async function save() {
  ...
  try {
    ...
    const result = form.value._id
      ? await post('/adminSaveCoach', { action: 'update', id: form.value._id, data })
      : await post('/adminSaveCoach', { action: 'add', data })
...
async function toggleStatus(row) {
  ...
  try {
    const result = await post('/adminSaveCoach', {
      action: 'toggle',
      id: row._id,
      data: { status: next }
    })
