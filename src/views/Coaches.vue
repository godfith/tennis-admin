<template>
  <div class="page">
    ...
    <el-table :data="list" stripe border v-loading="loading">
      ...
      <el-table-column prop="specialty" label="擅长" min-width="140" />
      <el-table-column prop="sort" label="排序" width="80" />
      <el-table-column label="状态" width="100"><template #default="{ row }"><el-tag :type="isActive(row) ? 'success' : 'info'" size="small">{{ isActive(row) ? '在职' : '停用' }}</el-tag></template></el-table-column>
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" @click="openEdit(row)">编辑</el-button>
          <el-button link type="warning" @click="toggleStatus(row)">{{ isActive(row) ? '停用' : '启用' }}</el-button>
          <el-button link type="danger" @click="onDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog v-model="visible" :title="form._id ? '编辑教练' : '新增教练'" width="440px" destroy-on-close>
...
const form = ref({ _id: '', name: '', phone: '', specialty: '', sort: 0, active: true, remark: '' })
...
function isActive(row) { return row.status === 'active' || row.status === '在职' || row.status === 1 || row.status === true }
...
function openAdd() {
  ...
  form.value = { _id: '', name: '', phone: '', specialty: '', sort: 0, active: true, remark: '' }
  visible.value = true
}
function openEdit(row) {
  form.value = { _id: row._id, name: row.name || '', phone: row.phone || '', specialty: row.specialty || '', sort: row.sort || 0, active: isActive(row), remark: row.remark || '' }
  visible.value = true
}
async function save() {
  ...
  saving.value = true
  try {
    const data = { name: form.value.name, phone: form.value.phone, specialty: form.value.specialty, sort: form.value.sort, status: form.value.active ? 'active' : 'disabled', remark: form.value.remark, venueId: venueId() }
    const result = form.value._id ? await post('/adminSaveCoach', { action: 'update', id: form.value._id, data }) : await post('/adminSaveCoach', { action: 'add', data })
...
async function toggleStatus(row) {
  const next = isActive(row) ? 'disabled' : 'active'
  const result = await post('/adminSaveCoach', { action: 'toggle', id: row._id, data: { status: next } })
  if (!result.ok) { ElMessage.error(result.msg || '操作失败'); return }
  ElMessage.success('已更新'); loadData()
}
async function onDelete(row) {
  try {
    await ElMessageBox.confirm('确定删除教练「' + row.name + '」？', '警告', { type: 'warning' })
    const result = await post('/adminSaveCoach', { action: 'delete', id: row._id })
